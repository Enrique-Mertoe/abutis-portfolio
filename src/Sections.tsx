const HeroSection = function () {
    return (
        <section className="bg-light">
            <div className="container">
                <div>
                    <div className="row py-8 align-items-center gy-8">
                        <div className="col-xl-5 col-lg-5">
                            <div>
                                <span className="text-primary fw-semibold">Welcome to the Geeks Abroad Study</span>
                                <h1 className="display-3 my-3">Powering Your Global Education Dream</h1>
                                <p className="mb-5 fs-3">Explore top universities and unique cultural experiences
                                    worldwide.</p>
                                <a href="#" className="btn btn-primary btn-lg">Enquire now</a>
                                <div className="mt-4 d-flex gap-3 align-items-center">
                                    <div className="avatar-group">
                      <span className="avatar avatar-md">
                          <img alt="avatar" src="../../assets/images/avatar/avatar-1.jpg" className="rounded-circle"/>
                      </span>
                                        <span className="avatar avatar-md">
                        <img alt="avatar" src="../../assets/images/avatar/avatar-2.jpg" className="rounded-circle"/>
                      </span>
                                        <span className="avatar avatar-md">
                        <img alt="avatar" src="../../assets/images/avatar/avatar-3.jpg" className="rounded-circle"/>
                      </span>
                                        <span className="avatar avatar-md">
                        <img alt="avatar" src="../../assets/images/avatar/avatar-4.jpg" className="rounded-circle"/>
                      </span>
                                    </div>
                                    <div className="">
                                        <div className="d-flex align-items-center gap-2 fs-4 lh-0 mb-1">
                                            <div className="text-dark fw-semibold">4.8</div>
                                            <div className="">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                     fill="currentColor" className="bi bi-star-fill text-warning"
                                                     viewBox="0 0 16 16">
                                                    <path
                                                        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>
                                                </svg>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                     fill="currentColor" className="bi bi-star-fill text-warning"
                                                     viewBox="0 0 16 16">
                                                    <path
                                                        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>
                                                </svg>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                     fill="currentColor" className="bi bi-star-fill text-warning"
                                                     viewBox="0 0 16 16">
                                                    <path
                                                        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>
                                                </svg>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                     fill="currentColor" className="bi bi-star-fill text-warning"
                                                     viewBox="0 0 16 16">
                                                    <path
                                                        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>
                                                </svg>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                     fill="currentColor" className="bi bi-star-fill text-warning"
                                                     viewBox="0 0 16 16">
                                                    <path
                                                        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>
                                                </svg>
                                            </div>
                                        </div>
                                        <div>Engaged Students</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="offset-xl-1 col-xl-6 col-lg-7 d-flex justify-content-center py-6">
                            <div className="text-center position-relative">
                                <div className="position-relative">
                                    <img src="assets/circle.png" alt=""
                                         className="position-relative img-fluid bg-invert"/>
                                    <div className="position-absolute top-0 ms-n8 mt-n6">
                                        <img src="../../assets/images/landing-immigration/plane.png" alt=""/>
                                    </div>
                                    <div className="position-absolute top-25 mt-4 z-1 ms-xl-n8 d-none d-md-block">
                                        <div
                                            className="bg-white border rounded-pill px-3 py-2 d-inline-block fw-medium shadow-lg">Highly
                                            Qualified Teachers
                                        </div>
                                    </div>
                                    <div
                                        className="position-absolute top-50 z-1 ms-n5 ms-xl-n8 mt-n4 d-none d-md-block">
                                        <div
                                            className="bg-white border rounded-pill px-3 py-2 d-inline-block fw-medium shadow-lg">Online
                                            and Offline classes
                                        </div>
                                    </div>
                                    <div className="position-absolute top-60 z-1 mt-2 ms-xl-n8 d-none d-md-block">
                                        <div
                                            className="bg-white border rounded-pill px-3 py-2 d-inline-block fw-medium shadow-lg">International
                                            Recognition
                                        </div>
                                    </div>
                                    <div className="position-absolute top-50 start-50 translate-middle">
                                        <svg width="315" height="315" viewBox="0 0 315 315" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <rect x="3.06323" y="2.99145" width="308.4" height="308.4"
                                                  rx="154.2" fill="white"></rect>
                                            <rect x="3.06323" y="2.99145" width="308.4" height="308.4"
                                                  rx="154.2" fill="#F20000"></rect>
                                            <rect x="3.06323" y="2.99145" width="308.4" height="308.4"
                                                  rx="154.2" stroke="var(--gk-gray-200)"
                                                  stroke-width="5.81886"></rect>
                                        </svg>
                                        <div className="position-absolute start-50 translate-middle"
                                             >
                                            <img src="../../assets/images/landing-immigration/girl.png" alt=""/>
                                        </div>
                                    </div>
                                    <div
                                        className="position-absolute top-0 start-50 ms-n8 mt-n8 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="35" viewBox="0 0 73 72" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3128)">
                                                <path
                                                    d="M36.4297 72C56.3119 72 72.4297 55.8823 72.4297 36C72.4297 16.1177 56.3119 0 36.4297 0C16.5474 0 0.429688 16.1177 0.429688 36C0.429688 55.8823 16.5474 72 36.4297 72Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M7.87186 14.079C5.04404 17.7582 2.91174 21.9987 1.67017 26.6055H20.3983L7.87186 14.079Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M71.1908 26.6059C69.9492 21.9993 67.8168 17.7588 64.9891 14.0796L52.4629 26.6059H71.1908Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M1.67017 45.3907C2.91189 49.9973 5.04418 54.2378 7.87187 57.9169L20.3979 45.3907H1.67017Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M58.3464 7.43933C54.6672 4.61151 50.4268 2.47921 45.8201 1.23749V19.9655L58.3464 7.43933Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M14.5117 64.5531C18.1909 67.3809 22.4314 69.5132 27.038 70.7549V52.027L14.5117 64.5531Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M27.0379 1.23749C22.4313 2.47921 18.1907 4.6115 14.5117 7.43919L27.0379 19.9654V1.23749Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M45.8201 70.7549C50.4267 69.5132 54.6672 67.3809 58.3462 64.5532L45.8201 52.027V70.7549Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M52.4629 45.3907L64.9891 57.917C67.8168 54.2379 69.9492 49.9973 71.1908 45.3907H52.4629Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M72.125 31.3044H41.1256H41.1254V0.304734C39.5883 0.104625 38.0211 0 36.4297 0C34.838 0 33.2711 0.104625 31.7341 0.304734V31.3041V31.3043H0.734422C0.534313 32.8414 0.429688 34.4085 0.429688 36C0.429688 37.5917 0.534313 39.1586 0.734422 40.6956H31.7338H31.7339V71.6953C33.2711 71.8954 34.838 72 36.4297 72C38.0211 72 39.5883 71.8955 41.1253 71.6953V40.6959V40.6957H72.125C72.3251 39.1586 72.4297 37.5917 72.4297 36C72.4297 34.4085 72.3251 32.8414 72.125 31.3044Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M45.8208 45.3938L61.8852 61.4582C62.6241 60.7196 63.3289 59.9475 64.0014 59.147L50.248 45.3936H45.8208V45.3938Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M27.0382 45.3936H27.0379L10.9736 61.4579C11.7122 62.1968 12.4844 62.9016 13.2848 63.574L27.0382 49.8204V45.3936Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M27.0377 26.6073V26.6071L10.9733 10.5425C10.2345 11.281 9.52965 12.0532 8.85718 12.8537L22.6107 26.6072H27.0377V26.6073Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M45.8208 26.6106L61.8854 10.5459C61.1468 9.80701 60.3747 9.1022 59.5742 8.42987L45.8208 22.1834V26.6106V26.6106Z"
                                                    fill="#D80027"></path>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_7973_3128">
                                                    <rect x="0.429688" width="72" height="72" rx="16.6253"
                                                          fill="white"></rect>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                    <div
                                        className="position-absolute top-0 start-50 ms-8 mt-n4 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="36" viewBox="0 0 73 73" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3131)">
                                                <path
                                                    d="M2.94214 49.2542C8.02869 62.9629 21.2242 72.7325 36.703 72.7325C52.1817 72.7325 65.3772 62.9629 70.4638 49.2542L36.703 46.1239L2.94214 49.2542Z"
                                                    fill="#FFDA44"></path>
                                                <path
                                                    d="M36.703 0.731812C21.2242 0.731812 8.02869 10.5013 2.94214 24.2101L36.703 27.3405L70.4638 24.21C65.3772 10.5013 52.1817 0.731812 36.703 0.731812Z"
                                                    fill="black"></path>
                                                <path
                                                    d="M2.94254 24.2068C1.49551 28.1067 0.703369 32.3249 0.703369 36.7285C0.703369 41.132 1.49551 45.3502 2.94254 49.2501H70.4643C71.9114 45.3502 72.7034 41.132 72.7034 36.7285C72.7034 32.3249 71.9114 28.1067 70.4642 24.2068H2.94254Z"
                                                    fill="#D80027"></path>
                                            </g>
                                        </svg>
                                    </div>
                                    <div
                                        className="position-absolute top-10 start-60 mt-8 ms-4 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="36" viewBox="0 0 73 73" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3127)">
                                                <path
                                                    d="M72.9531 36.9873C72.9531 56.8694 56.8353 72.9873 36.9531 72.9873C17.071 72.9873 0.953125 56.8694 0.953125 36.9873C0.953125 36.9957 36.9531 0.991383 36.9531 0.987305C56.8353 0.987305 72.9531 17.1052 72.9531 36.9873Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M36.8564 36.9885H36.953C36.953 36.9558 36.953 36.9246 36.953 36.892C36.9208 36.9243 36.8887 36.9564 36.8564 36.9885Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M36.9524 19.7723C36.9524 13.4379 36.9524 9.28704 36.9524 0.989746H36.9463C17.0669 0.99298 0.952393 17.1093 0.952393 36.9896H19.735V26.4129L30.3116 36.9896H36.8562C36.8884 36.9575 36.9204 36.9255 36.9525 36.8933C36.9525 34.4679 36.9525 32.3038 36.9525 30.3491L26.3757 19.7723H36.9524Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M19.1666 5.68442C13.5361 8.89053 8.85528 13.5714 5.64917 19.2019V36.9888H15.0405V15.0759V15.0758H36.9536C36.9536 12.1138 36.9536 9.29201 36.9536 5.68442H19.1666V5.68442Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M36.9543 32.562L24.1641 19.7719H19.7371C19.7371 19.7717 19.7371 19.7719 19.7371 19.7719L36.9542 36.989H36.9543C36.9543 36.989 36.9543 33.9367 36.9543 32.562Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M22.666 43.2466L24.6418 47.3779L29.1034 46.3467L27.1054 50.4673L30.6933 53.3126L26.226 54.3195L26.2385 58.8988L22.666 56.0338L19.0935 58.8988L19.106 54.3195L14.6387 53.3126L18.2267 50.4673L16.2284 46.3467L20.6903 47.3779L22.666 43.2466Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M54.8544 51.0734L55.8423 53.139L58.0732 52.6234L57.074 54.6837L58.8681 56.1064L56.6344 56.6098L56.6406 58.8995L54.8544 57.4669L53.0682 58.8995L53.0744 56.6098L50.8408 56.1064L52.6348 54.6837L51.6358 52.6234L53.8665 53.139L54.8544 51.0734Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M45.6636 29.1615L46.6514 31.2273L48.8822 30.7115L47.8832 32.7718L49.6772 34.1945L47.4436 34.6979L47.4497 36.9876L45.6636 35.555L43.8774 36.9876L43.8834 34.6979L41.6499 34.1945L43.4439 32.7718L42.4449 30.7115L44.6757 31.2273L45.6636 29.1615Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M54.8544 16.6401L55.8423 18.7059L58.0732 18.1902L57.0742 20.2505L58.868 21.6731L56.6344 22.1767L56.6406 24.4663L54.8544 23.0338L53.0682 24.4663L53.0744 22.1767L50.8408 21.6731L52.6346 20.2505L51.6358 18.1902L53.8665 18.7059L54.8544 16.6401Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M62.8799 26.0306L63.8677 28.0964L66.0985 27.5806L65.0995 29.6409L66.8935 31.0636L64.6598 31.5672L64.666 33.8567L62.8799 32.4243L61.0937 33.8567L61.0998 31.5672L58.8662 31.0636L60.6602 29.6409L59.6612 27.5806L61.8919 28.0964L62.8799 26.0306Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M57.141 36.9883L57.9179 39.3798H60.4326L58.3982 40.8579L59.1754 43.2492L57.141 41.7712L55.1067 43.2492L55.8837 40.8579L53.8494 39.3798H56.3639L57.141 36.9883Z"
                                                    fill="#F0F0F0"></path>
                                            </g>
                                        </svg>
                                    </div>
                                    <div
                                        className="position-absolute top-10 end-10 end-md-10 end-lg-0 me-md-n4 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="36" viewBox="0 0 73 73" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3133)">
                                                <path
                                                    d="M36.4492 72.501C56.3315 72.501 72.4492 56.3832 72.4492 36.501C72.4492 16.6187 56.3315 0.500977 36.4492 0.500977C16.567 0.500977 0.449219 16.6187 0.449219 36.501C0.449219 56.3832 16.567 72.501 36.4492 72.501Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M8.67798 13.5902H64.2204C62.1689 11.1061 59.7895 8.90319 57.1502 7.04483H15.7482C13.1088 8.90291 10.7296 11.1059 8.67798 13.5902Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M0.449219 36.5026C0.449219 37.6061 0.501672 38.6971 0.598844 39.7754H72.2996C72.3968 38.6971 72.4492 37.6061 72.4492 36.5026C72.4492 35.3991 72.3968 34.3082 72.2996 33.23H0.598844C0.501672 34.3082 0.449219 35.3991 0.449219 36.5026Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M4.37573 52.8648H68.5243C69.5844 50.7912 70.4489 48.6015 71.0944 46.3196H1.80566C2.45099 48.6015 3.31584 50.7913 4.37573 52.8648Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M15.7482 65.9552H57.15C59.7894 64.0969 62.1688 61.894 64.2202 59.4098H8.67798C10.7296 61.8937 13.1088 64.0969 15.7482 65.9552Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M1.80542 26.6846H71.0944C70.449 24.4025 69.5843 22.2127 68.5242 20.1392H4.37562C3.31573 22.2127 2.45089 24.4025 1.80542 26.6846Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M17.6648 7.04655H57.1484C51.2927 2.92371 44.1532 0.50116 36.4474 0.50116C31.7518 0.50116 23.5204 2.92371 17.6648 7.04655Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M34.8838 20.1368H68.5232C67.3261 17.7949 65.8802 15.6013 64.2201 13.5914H34.8838V20.1368Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M34.8838 33.2264H72.2993C72.0968 30.9789 71.6899 28.7903 71.0935 26.681H34.8838V33.2264Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M1.80366 46.3219H71.0924C71.6889 44.2128 72.0959 42.0239 72.2984 39.7765H0.597656C0.800156 42.0239 1.20713 44.2126 1.80366 46.3219Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M8.67715 59.4115H64.2195C65.8796 57.4013 67.3255 55.2078 68.5227 52.8661H4.37402C5.57116 55.208 7.01707 57.4016 8.67715 59.4115Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M36.4471 72.5024C44.1529 72.5024 51.2924 70.0798 57.1481 65.9569H15.7461C21.6017 70.0798 28.7412 72.5024 36.4471 72.5024Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M1.80474 26.6815C1.2082 28.7909 0.801377 30.9795 0.598877 33.2269H36.4492V0.499695C19.9708 0.499695 6.07861 11.5714 1.80474 26.6815Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M21.8802 9.89255L24.211 17.0665H31.755L25.6521 21.5007L27.9829 28.6751L21.8802 24.2412L15.7772 28.6751L18.1085 21.5007L12.0056 17.0665H19.5492L21.8802 9.89255Z"
                                                    fill="#F0F0F0"></path>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_7973_3133">
                                                    <rect x="0.449219" y="0.500977" width="72" height="72"
                                                          rx="36" fill="white"></rect>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                    <div
                                        className="position-absolute top-25 end-0 end-xl-20 me-xl-n8 mt-8 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="35" viewBox="0 0 73 72" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3128)">
                                                <path
                                                    d="M36.4297 72C56.3119 72 72.4297 55.8823 72.4297 36C72.4297 16.1177 56.3119 0 36.4297 0C16.5474 0 0.429688 16.1177 0.429688 36C0.429688 55.8823 16.5474 72 36.4297 72Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M7.87186 14.079C5.04404 17.7582 2.91174 21.9987 1.67017 26.6055H20.3983L7.87186 14.079Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M71.1908 26.6059C69.9492 21.9993 67.8168 17.7588 64.9891 14.0796L52.4629 26.6059H71.1908Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M1.67017 45.3907C2.91189 49.9973 5.04418 54.2378 7.87187 57.9169L20.3979 45.3907H1.67017Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M58.3464 7.43933C54.6672 4.61151 50.4268 2.47921 45.8201 1.23749V19.9655L58.3464 7.43933Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M14.5117 64.5531C18.1909 67.3809 22.4314 69.5132 27.038 70.7549V52.027L14.5117 64.5531Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M27.0379 1.23749C22.4313 2.47921 18.1907 4.6115 14.5117 7.43919L27.0379 19.9654V1.23749Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M45.8201 70.7549C50.4267 69.5132 54.6672 67.3809 58.3462 64.5532L45.8201 52.027V70.7549Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M52.4629 45.3907L64.9891 57.917C67.8168 54.2379 69.9492 49.9973 71.1908 45.3907H52.4629Z"
                                                    fill="#0052B4"></path>
                                                <path
                                                    d="M72.125 31.3044H41.1256H41.1254V0.304734C39.5883 0.104625 38.0211 0 36.4297 0C34.838 0 33.2711 0.104625 31.7341 0.304734V31.3041V31.3043H0.734422C0.534313 32.8414 0.429688 34.4085 0.429688 36C0.429688 37.5917 0.534313 39.1586 0.734422 40.6956H31.7338H31.7339V71.6953C33.2711 71.8954 34.838 72 36.4297 72C38.0211 72 39.5883 71.8955 41.1253 71.6953V40.6959V40.6957H72.125C72.3251 39.1586 72.4297 37.5917 72.4297 36C72.4297 34.4085 72.3251 32.8414 72.125 31.3044Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M45.8208 45.3938L61.8852 61.4582C62.6241 60.7196 63.3289 59.9475 64.0014 59.147L50.248 45.3936H45.8208V45.3938Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M27.0382 45.3936H27.0379L10.9736 61.4579C11.7122 62.1968 12.4844 62.9016 13.2848 63.574L27.0382 49.8204V45.3936Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M27.0377 26.6073V26.6071L10.9733 10.5425C10.2345 11.281 9.52965 12.0532 8.85718 12.8537L22.6107 26.6072H27.0377V26.6073Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M45.8208 26.6106L61.8854 10.5459C61.1468 9.80701 60.3747 9.1022 59.5742 8.42987L45.8208 22.1834V26.6106V26.6106Z"
                                                    fill="#D80027"></path>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_7973_3128">
                                                    <rect x="0.429688" width="72" height="72" rx="16.6253"
                                                          fill="white"></rect>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                    <div
                                        className="position-absolute top-50 end-0 mt-8 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="36" viewBox="0 0 73 73" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3132)">
                                                <path
                                                    d="M36.9805 72.5234C56.8627 72.5234 72.9805 56.4057 72.9805 36.5234C72.9805 16.6412 56.8627 0.523438 36.9805 0.523438C17.0982 0.523438 0.980469 16.6412 0.980469 36.5234C0.980469 56.4057 17.0982 72.5234 36.9805 72.5234Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M72.9831 36.5266C72.9831 22.254 64.6771 9.92176 52.6353 4.09875V68.9543C64.6771 63.1314 72.9831 50.7992 72.9831 36.5266Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M0.981934 36.5246C0.981934 50.7972 9.28795 63.1295 21.3298 68.9525V4.0968C9.28795 9.9198 0.981934 22.252 0.981934 36.5246Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M43.243 41.2221L49.5038 38.0916L46.3735 36.5265V33.396L40.1126 36.5265L43.243 30.2656H40.1126L36.9821 25.5699L33.8517 30.2656H30.7212L33.8517 36.5265L27.5908 33.396V36.5265L24.4604 38.0916L30.7212 41.2221L29.1561 44.3525H35.417V49.0481H38.5473V44.3525H44.8082L43.243 41.2221Z"
                                                    fill="#D80027"></path>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_7973_3132">
                                                    <rect x="0.980469" y="0.523438" width="72" height="72"
                                                          rx="36" fill="white"></rect>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                    <div
                                        className="position-absolute top-75 start-60 ms-n8 pt-3 d-none d-md-block bg-white p-3 rounded-circle shadow-lg border">
                                        <svg width="36" height="36" viewBox="0 0 73 73" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g clip-path="url(#clip0_7973_3129)">
                                                <path
                                                    d="M36.8789 72.0742C56.7612 72.0742 72.8789 55.9565 72.8789 36.0742C72.8789 16.192 56.7612 0.0742188 36.8789 0.0742188C16.9967 0.0742188 0.878906 16.192 0.878906 36.0742C0.878906 55.9565 16.9967 72.0742 36.8789 72.0742Z"
                                                    fill="#F0F0F0"></path>
                                                <path
                                                    d="M35.313 36.0742H72.8781C72.8781 32.8249 72.445 29.6772 71.638 26.6829H35.313V36.0742Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M35.313 17.2898H67.5943C65.3906 13.6937 62.5729 10.5152 59.2876 7.89844H35.313V17.2898Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M36.8782 72.0743C45.3507 72.0743 53.1381 69.1459 59.2876 64.2482H14.4688C20.6183 69.1459 28.4057 72.0743 36.8782 72.0743Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M6.16208 54.854H67.5944C69.3636 51.9671 70.7357 48.8115 71.6381 45.4626H2.11841C3.0208 48.8115 4.39288 51.9671 6.16208 54.854Z"
                                                    fill="#D80027"></path>
                                                <path
                                                    d="M17.5548 5.69613H20.8354L17.7839 7.91308L18.9495 11.5003L15.8981 9.28333L12.8467 11.5003L13.8535 8.40133C11.1667 10.6394 8.81184 13.2615 6.87122 16.1831H7.92239L5.97994 17.5943C5.67731 18.0991 5.38706 18.612 5.10891 19.1324L6.03647 21.9873L4.30594 20.7299C3.87577 21.6413 3.4823 22.5732 3.12863 23.5246L4.15055 26.6701H7.92239L4.87083 28.887L6.03647 32.4742L2.98505 30.2573L1.1572 31.5853C0.97425 33.056 0.878906 34.5539 0.878906 36.0742H36.8789C36.8789 16.1921 36.8789 13.8482 36.8789 0.0742188C29.7672 0.0742188 23.1377 2.13719 17.5548 5.69613ZM18.9495 32.4742L15.8981 30.2573L12.8467 32.4742L14.0123 28.887L10.9607 26.6701H14.7326L15.8981 23.0829L17.0636 26.6701H20.8354L17.7839 28.887L18.9495 32.4742ZM17.7839 18.4L18.9495 21.9873L15.8981 19.7703L12.8467 21.9873L14.0123 18.4L10.9607 16.1831H14.7326L15.8981 12.5959L17.0636 16.1831H20.8354L17.7839 18.4ZM31.8625 32.4742L28.8111 30.2573L25.7597 32.4742L26.9253 28.887L23.8738 26.6701H27.6456L28.8111 23.0829L29.9766 26.6701H33.7485L30.6969 28.887L31.8625 32.4742ZM30.6969 18.4L31.8625 21.9873L28.8111 19.7703L25.7597 21.9873L26.9253 18.4L23.8738 16.1831H27.6456L28.8111 12.5959L29.9766 16.1831H33.7485L30.6969 18.4ZM30.6969 7.91308L31.8625 11.5003L28.8111 9.28333L25.7597 11.5003L26.9253 7.91308L23.8738 5.69613H27.6456L28.8111 2.10892L29.9766 5.69613H33.7485L30.6969 7.91308Z"
                                                    fill="#0052B4"></path>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_7973_3129">
                                                    <path
                                                        d="M0.878906 26.6747C0.878906 11.9837 12.7884 0.0742188 27.4794 0.0742188H46.2784C60.9695 0.0742188 72.8789 11.9837 72.8789 26.6747V45.4737C72.8789 60.1648 60.9695 72.0742 46.2784 72.0742H27.4794C12.7884 72.0742 0.878906 60.1648 0.878906 45.4737V26.6747Z"
                                                        fill="white"></path>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    );
}

export {
    HeroSection
}